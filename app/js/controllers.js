...
reddimgurApp.controller('ImgurCtrl', ['$scope', 'Imgur', 'Reddit',
  function ($scope, Imgur, Reddit) {
    ...
    $scope.reddit = Reddit.query();
    $scope.reddit.$promise.then( function(result) {
      $scope.currentSub = result.data.children[0];
    });
    $scope.pics = Imgur.query();
    $scope.pics.$promise.then( function(result) {
      formatPics(result);
    });
    ...
    $scope.selectSub = function(sub) {
      ...
      $scope.pics = Imgur.query({'subreddit': sub.data.url});
      $scope.pics.$promise.then( function(result) {
        formatPics(result);
      });
    };

    $scope.enhance = function(pic) {
      if (pic.span > 4) {
        return 4;
      } else {
        return pic.span;
    ...
    function formatPics(obj) {
      for (var pic in obj.data) {
        var link = obj.data[pic].link;
        var dot = link.indexOf(".", 20);
        var size = randomSize();

        obj.data[pic].thumb = [link.slice(0, dot), size, link.slice(dot)].join('');
        obj.data[pic].color = randomColor();
        obj.data[pic].span = getSpan(size);
      }
    }
